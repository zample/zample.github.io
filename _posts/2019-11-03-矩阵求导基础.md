...
简单的求导的确不难，但许多用矩阵表达的函数并不容易拆分成各元素的函数，例如 $y = |X|$，或是用矩阵向量表达有更简洁的形式，此时再用定义法求解导数过程会很复杂，我们需要使用新的方法来求解矩阵向量求导。
...
- 行列式微分：$d|X|=\operatorname{tr}\left(X^{\#} d X\right)$，其中 $X^{\#}$ 是 $X$ 的伴随矩阵。当 $X$ 可逆时，$d|X|=|X| \operatorname{tr}\left(X^{-1} d X\right)$。
...
$$
**例5**：多元正态分布方差极大似然估计。样本 $\boldsymbol{x}_{1}, \dots, \boldsymbol{x}_{N} \sim \mathcal{N}(\boldsymbol{\mu}, \Sigma)$，求 $\Sigma$ 的极大似然估计。

**解**：由于多元正态密度函数为：
$$
p(\pmb x ; \pmb \mu, \Sigma)=\frac{1}{(2 \pi)^{n / 2}|\Sigma|^{1 / 2}} \exp \left(-\frac{1}{2}(\pmb x-\pmb\mu)^{T} \Sigma^{-1}(\pmb x-\pmb \mu)\right)
$$
最大化对数似然函数等价于最小化：
$$
l=\log |\Sigma|+\frac{1}{N} \sum_{i=1}^{N}\left(\boldsymbol{x}_{i}-\overline{\boldsymbol{x}}\right)^{T} \Sigma^{-1}\left(\boldsymbol{x}_{i}-\overline{\boldsymbol{x}}\right)
$$
首先求微分，第一项是
$$
...
\end{align}
$$
其中 $S=\frac{1}{N} \sum_{i=1}^{N}\left(\boldsymbol{x}_{i}-\overline{\boldsymbol{x}}\right)\left(\boldsymbol{x}_{i}-\overline{\boldsymbol{x}}\right)^{T} $ 。
